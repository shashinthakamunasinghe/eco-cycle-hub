// Simple script to create recent activity data using client Firebase
// This would normally be run from within the app or via an API call

import { collection, addDoc, Timestamp } from 'firebase/firestore';
import { db } from '../lib/firebase.js';

export async function createRecentActivityData() {
  try {
    ...
    const shippedOrderData = {
      ...
      createdAt: Timestamp.fromDate(new Date(now.getTime() - 10 * 60 * 1000))
    };
    
    const shippedOrderRef = await addDoc(collection(db, 'orders'), shippedOrderData);
