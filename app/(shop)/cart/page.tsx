...
export default function CartPage() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const stockValidationPromises = cartItems.map(async (cartItem) => {
        try {
          ...
          if (currentProduct.stock < cartItem.quantity) {
            throw new Error(
              `Insufficient stock for ${cartItem.name}. Available: ${currentProduct.stock}, In cart: ${cartItem.quantity}`
            );
          }
      ...
      const processedCartItems = cartItems.map((item) => {
        ...
        if (imageUrl) {
          try {
            ...
            // If it fails, it's a relative URL - convert to absolute
      ...
      console.log(
        `Sending order data: ${cartItems.length} items, total: $${total.toFixed(
          2
        )}`
      );
      ...
      if (error instanceof Error) {
        if (error.message.includes("Stripe") && error.message.includes("key")) {
          errorMessage =
            "Payment configuration error: Stripe API keys are not properly set up.";
          actionMessage =
            "Please ensure valid Stripe API keys are in .env.local file.";
        } else if (
          ...
          actionMessage =
            "This may be due to network issues or server configuration.";
        } else {
